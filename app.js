...
C('Armorer','갑옷장이','village',3,['craft'],'같은 행 또는 열의 군사 방패당 3점',{type:'crossShield',shield:'military',per:3},{effect:'이후 모든 카드 비용 -1'}),
...
C('Banker','은행가','castle',7,['craft'],'모든 지갑에 저장된 금 1개당 1점',{type:'allPurses',per:1},{effect:'각 지갑에 금 2 또는 열쇠 3'}),
C('Barbarian','야만인','village',2,['military'],'학문 방패가 없으면 10점',{type:'missingShield',shield:'scholar',points:10}),
C('Baron','남작','castle',3,['noble'],'농민 방패가 없으면 10점',{type:'missingShield',shield:'peasant',points:10},{effect:'이후 모든 카드 비용 -1'}),
C('Beekeeper','양봉가','village',2,['peasant'],'이 지갑의 금 1개당 2점',{type:'purse',per:2},{purse:9,effect:'각 지갑에 금 2'}),
...
C('Brigand','산적','village',7,['peasant'],'마을 카드 3장 세트당 7점',{type:'setsPlace',place:'village',size:3,per:7}),
C('Captain','대장','castle',5,['military','military'],'오른쪽 열이면 8점',{type:'position',positions:['right'],points:8},{effect:'이후 마을 카드 비용 -1'}),
...
C('Chaplain','성직자','castle',5,['faith'],'마을 카드당 2점',{type:'placeCards',place:'village',per:2}),
C('Chatelaine','성의 여주인','castle',2,['noble','craft'],'같은 행의 서로 다른 방패 종류당 2점',{type:'distinctLine',axis:'row',per:2},{effect:'이후 성 카드 비용 -1'}),
C('Clockmaker','시계공','village',3,['craft'],'같은 행의 장인 방패당 3점',{type:'lineShield',axis:'row',shield:'craft',per:3}),
C('Devout','독실한 신자','castle',4,['faith'],'장인 방패가 없으면 10점',{type:'missingShield',shield:'craft',points:10}),
...
C('Executioner','사형집행인','village',0,['military'],'성 카드당 1점',{type:'placeCards',place:'castle',per:1}),
...
C('Glassblower','유리공','castle',5,['faith','craft'],'신앙+장인 방패 세트당 4점',{type:'shieldSet',shields:['faith','craft'],per:4}),
C('Goldsmith','금세공인','castle',4,['scholar','craft'],'왼쪽 열이면 6점',{type:'position',positions:['left'],points:6}),
C('Gravedigger','무덤지기','castle',4,['faith','scholar'],'이 지갑의 금 1개당 2점',{type:'purse',per:2},{purse:8}),
C('Guildmaster','길드장','castle',5,['craft','craft'],'맨 아래 행이면 5점',{type:'position',positions:['bottom'],points:5}),
C('Her Majesty','여왕','castle',7,['noble','noble'],'귀족+학문+장인 방패 세트당 10점',{type:'shieldSet',shields:['noble','scholar','craft'],per:10}),
C('His Holiness','교황','castle',7,['faith'],'없는 방패 종류당 6점',{type:'missingTypes',per:6}),
C('His Majesty','국왕','castle',6,['noble','faith'],'같은 열의 귀족 방패당 4점',{type:'lineShield',axis:'col',shield:'noble',per:4}),
C('Innkeeper','여관주인','village',0,['craft'],'이 지갑의 금 1개당 2점',{type:'purse',per:2},{purse:6}),
...
C('Judge','판사','castle',4,['scholar'],'성+마을 카드 쌍당 3점',{type:'placePair',per:3}),
C('Knight','기사','castle',5,['military'],'같은 행 또는 열의 귀족 방패당 3점',{type:'crossShield',shield:'noble',per:3}),
C('Locksmith','자물쇠공','village',4,['craft','peasant'],'보유 열쇠 1개당 추가 1점',{type:'keys',per:1},{effect:'장인 방패당 열쇠 1'}),
C('Lookout','망지기','castle',6,['military','military'],'같은 열의 서로 다른 방패 종류당 4점',{type:'distinctLine',axis:'col',per:4}),
C('Master-at-Arms','무예교관','village',2,['military','military'],'이 지갑의 금 1개당 2점',{type:'purse',per:2},{purse:4}),
...
C('Miraculously Cured','기적적으로 치유된 사람','village',2,['faith','faith'],'이 지갑의 금 1개당 2점',{type:'purse',per:2},{purse:4}),
C('Monk','수도사','village',4,['peasant','faith'],'같은 행 또는 열의 농민 방패당 2점',{type:'crossShield',shield:'peasant',per:2}),
...
C('Pawnbroker','전당포 주인','castle',4,['craft'],'인쇄 비용 4인 카드당 3점',{type:'costExact',cost:4,per:3}),
...
C('Potter','도공','village',2,['craft','craft'],'이 지갑의 금 1개당 2점',{type:'purse',per:2},{purse:4}),
...
C('Queen Mother','대비','castle',3,['noble','noble'],'이 지갑의 금 1개당 2점',{type:'purse',per:2},{purse:5}),
C('Revolutionary','혁명가','village',4,['peasant'],'귀족 방패가 없으면 9점',{type:'missingShield',shield:'noble',points:9}),
C('Royal Guard','왕실 근위병','castle',4,['noble','military'],'같은 열의 귀족 방패당 3점',{type:'lineShield',axis:'col',shield:'noble',per:3}),
C('Scribe','서기관','castle',4,['faith'],'같은 행 또는 열의 학문 방패당 3점',{type:'crossShield',shield:'scholar',per:3}),
...
C('Stable Boy','마구간지기','village',4,['noble','peasant'],'같은 열의 농민 방패당 3점',{type:'lineShield',axis:'col',shield:'peasant',per:3}),
C('Steward','집사','castle',0,['noble'],'이 지갑의 금 1개당 2점',{type:'purse',per:2},{purse:3}),
C('Stonemason','석공','village',3,['craft'],'같은 열의 장인 방패당 3점',{type:'lineShield',axis:'col',shield:'craft',per:3},{effect:'이후 마을 카드 비용 -1'}),
C('Templar','성전기사','castle',5,['faith','military'],'보유 열쇠 1개당 추가 1점',{type:'keys',per:1}),
...
C('Slimeball','수상한 자','castle',7,['noble'],'같은 행 3장의 인쇄 비용 합계',{type:'rowCostSum'},{expansion:true,dutch:'Slijmbal',lock:true}),
...
C('Dreamer with the Iron Mask','철가면의 몽상가','castle',8,['noble','faith','scholar'],'서로 다른 인쇄 비용 종류당 3점',{type:'distinctCosts',per:3},{expansion:true,dutch:'Dromer met het ijzeren masker',lock:true}),
C('Actor','배우','castle',3,['scholar'],'성 카드 1장당 2점',{type:'placeCards',place:'castle',per:2},{expansion:true,dutch:'Toneelspeler',lock:true}),
...
function renderCardList(){const q=search.value.trim().toLowerCase();const used=new Set(state.slots.map((s,i)=>i===activeSlot?null:s.cardId));const list=cards.filter(c=>(filter==='all'||(filter==='base'?!c.expansion:(filter==='expansion'?c.expansion:c.place===filter)))&&(!q||`${c.name} ${c.ko} ${c.dutch||''}`.toLowerCase().includes(q)));cardList.innerHTML=list.map(c=>`<button type="button" class="card-option" data-id="${c.id}" ${used.has(c.id)?'disabled':''}><strong>${c.expansion?`${c.dutch} · ${c.ko}`:`${c.name} · ${c.ko}`}</strong><small>${c.expansion?`${c.name} · 확장팩`:'기본판'} · ${c.place==='castle'?'성':'마을'} / 비용 ${c.cost} · ${c.formula}${used.has(c.id)?' · 이미 선택됨':''}</small><div class="mini-shields">${c.shields.map(x=>`<span class="shield ${x}">${S[x]}</span>`).join('')}</div></button>`).join('');cardList.querySelectorAll('.card-option:not([disabled])').forEach(b=>b.addEventListener('click',()=>{state.slots[activeSlot]={cardId:b.dataset.id,faceDown:false,purse:0,lockUnused:true};dialog.close();save();render();}))}
init();
